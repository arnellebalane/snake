// Okay so far, but grid cells should be cell objects so that they can
// be added to a linked-list or something so that they can remember their
// parent cell
...
var human = {
  ...
  initialize: function() {
    ...
    human.snake = new Snake(5, snakeHead, "right");
    human.controls();
  },
  controls: function() {
...
var computer = {
  ...
  initialize: function() {
    ...
    computer.snake = new Snake(5, snakeHead, "left");
  },
  cycle: function() {
    while (computer.path.length == 0) {
      computer.findPath();
    }
    var move = computer.path.shift();
    computer.snake.changeDirection(move);
  ...
  findPath: function() {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      if ((move == "up" && computer.snake.moveDirection != "down" && computer.snake.segments[0].y > 1)
          || (move == "left" && computer.snake.moveDirection != "right" && computer.snake.segments[0].x > 1)
          || (move == "right" && computer.snake.moveDirection != "left" && computer.snake.segments[0].x < game.grid.width - 2)
          || (move == "down" && computer.snake.moveDirection != "up" && computer.snake.segments[0].y < game.grid.height - 2)) {
        computer.path.push(move);
      }
    }
  }
};
...
function Snake(length, headSegment, moveDirection) {
  ...
  this.initialize = function() {
    ...
    while (length-- > 1) {
      ...
      var segment = null;
      if (this.moveDirection == "up" && last.y + 1 < game.grid.height) {
        segment = new Segment(last.x, last.y + 1);
      } else if (this.moveDirection == "left" && last.x + 1 < game.grid.width) {
        segment = new Segment(last.x + 1, last.y);
      } else if (this.moveDirection == "right" && last.x - 1 >= 0) {
        segment = new Segment(last.x - 1, last.y);
      } else if (this.moveDirection == "down" && last.y - 1 >= 0) {
        segment = new Segment(last.x, last.y - 1);
      }
      if (segment != null) {
